...
| 2026-06-19 | **Fix compresión + client tests.** Bug: Node.js `compression()` middleware en `server.js` hacía que el gateway recibiera respuestas gzip sin decodificar → JSON parse fallaba en upload. Fix en `portal_server.py`: excluir `accept-encoding` de headers reenviados (el gateway maneja descompresión). Nuevo `src/__tests__/client.test.ts` (24 tests): URLs correctas de todos los endpoints, exportaciones, handling de errores). **Total: 81/81 tests** (stepbar 26 + uploadzone 31 + client 24). |
| 2026-06-25 | **Fix upload zone — tarjetas desniveladas.** Filenames largos causaban que `.uz-file` se extendiera a 2 líneas → tarjetas de altura desigual en la pantalla de carga de Job Matcher. Fix CSS en `src/index.css`: `.upload-zone .uz-file` con `max-width: 100%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; padding: 0 8px`. `UploadZone.tsx`: `<span className="uz-file" title={filename}>` para mostrar nombre completo en hover tooltip. **84/84 tests** (sin cambio de conteo — fix es solo CSS + HTML attr). |
