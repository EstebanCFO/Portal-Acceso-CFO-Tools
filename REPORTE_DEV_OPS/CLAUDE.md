...
| 2026-06-29 | **Cronograma de sprints + rediseño compacto.** Backend: `/api/sprint-report` suma `allSprints` (todos los sprints con `estado`=timeFrame, sin llamadas extra a Azure). Fix red corporativa: `truststore.inject_into_ssl()` en `app.py` (TLS interceptado → `CERTIFICATE_VERIFY_FAILED`; combos vacíos). Frontend rediseñado bajo nueva **premisa del DS** (informe compacto): `SprintHero` banda navy con anillo+semáforo de riesgo, cronograma como franja horizontal `.rdo-crono`, grid 2 columnas (actual `1.35fr` / anterior `1fr`), Test Plan en barras finas, work items colapsados por defecto. Tipos `SprintSummary`/`SprintEstado`. **65/65 tests** + typecheck OK. |
