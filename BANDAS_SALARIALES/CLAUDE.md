...
| 2026-06-16 | **Fix iframe (gateway):** Eliminado `vite.config.js` legacy (sin `base`) que Vite resolvía con prioridad sobre `vite.config.ts` (con `base: '/apps/bandas-salariales/'`), impidiendo que las apps cargaran en el portal. Rebuilt `dist/` — todos los assets ahora usan el prefijo correcto. Fix `setup.ts`: `global.fetch` → `(globalThis as unknown as {...}).fetch` (sin `@types/node`). **114/114** tests pasan. |
| 2026-06-19 | **Fix BrowserRouter basename:** `BrowserRouter` sin `basename` en `App.tsx` causaba que React Router viera `/apps/bandas-salariales/` como path no reconocido → navigate implícito a `/` en la instancia iframe (sin recarga). Fix: `basename="/apps/bandas-salariales"` para que las rutas relativas (/, /tabla, /historial, /empleado) se resuelvan correctamente. Rebuilt dist. **114/114** tests pasan. |
